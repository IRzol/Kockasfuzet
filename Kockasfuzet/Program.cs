...
namespace Kockasfuzet
{
    ...
    {
        // Globális változók a menüvezérléshez
        static int cPoint = 0;
        static ConsoleColor activeForeground = ConsoleColor.White;
        static ConsoleColor activeBack = ConsoleColor.DarkGray; // Háttérszín a kijelölésnél

        static void Main(string[] args)
        {
            // Konzol alapbeállítások a szép megjelenéshez
        ...
        static void Fomenu()
        {
            ...
            {
                bool selected = false;
                // --- MENÜ MEGJELENÍTÉSE ÉS NAVIGÁCIÓ ---
                do
                {
                    ShowMenu(cPoint);
                    ConsoleKeyInfo keyInfo = Console.ReadKey(true); // true = ne írja ki a karaktert
                    ...
                    switch (keyInfo.Key)
                    {
                        ...
                        case ConsoleKey.DownArrow:
                            if (cPoint < 3) cPoint++; // 0,1,2,3 (összesen 4 menüpont)
                            break;
                        case ConsoleKey.Escape: // Escape-re is lehessen kilépni
                            cPoint = 3;
                            selected = true;
                            break;
                    }
                ...
                // --- KIVÁLASZTOTT MENÜPONT VÉGREHAJTÁSA ---
                Console.CursorVisible = true; // Adatbevitelhez visszakapcsoljuk a kurzort
                Console.Clear();
                ...
                Console.CursorVisible = false; // Menühöz újra elrejtjük
        ...
        static void ShowMenu(int cPoint)
        {
            ...
            // Segédfüggvény a menüpontok kiírásához
        ...
        // --- LOGIKA: SZOLGÁLTATÓK (Eredeti kód 1-es case) ---
        ...
        // --- LOGIKA: SZOLGÁLTATÁSOK (Eredeti kód 2-es case) ---
        ...
        // --- LOGIKA: SZÁMLÁK (Eredeti kód 3-as case) ---
        static void KezelSzamlak()
        {
            ...
            switch (altvalasz)
            {
                case "1":
                    ...
                    {
                        Console.Clear();
                        // Segítségképpen listázzuk a szolgáltatásokat
